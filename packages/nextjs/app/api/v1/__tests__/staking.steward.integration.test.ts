...
describe("staking steward listing + x402 integration", () => {
  it("discovers staking steward and runs paid stake execution", async () => {
    clearAgentProfiles();
    clearHires();
    clearRateLimits();

    const endpoint = "https://agents.cloak.local/staking-steward";
    ...
    const discoverReq = new NextRequest(
      "http://localhost/api/v1/marketplace/discover?agent_type=staking_steward",
      {
        method: "GET",
    ...
    expect(discovered.agents[0].agent_id).toBe("staking_steward");
    ...
    const hireReq = new NextRequest("http://localhost/api/v1/marketplace/hires", {
      ...
      body: JSON.stringify({
        agent_id: "staking_steward",
        operator_wallet: "0xabc123",
        policy_snapshot: { maxStakeAmount: "1000" },
    ...
    const runBody = {
      hire_id: hire.id,
      action: "stake",
      params: { amount: "100", pool: "0xpool1" },
      billable: true,
    };
    ...
    const payment = {
      ...
      proof: "proof-staking",
      replayKey: "rk_staking_1",
      contextHash: challenge.contextHash,
      expiresAt: challenge.expiresAt,
      nonce: "nonce_staking_run_1",
    ...
    expect(run.result.protocol).toBe("starkzap-staking");
    expect(run.payment_ref).toBe("pay_rk_staking_1");
  });
});
