...
describe("marketplace runs route", () => {
  ...
  it("persists settlement tx hash from strict x402 envelope", async () => {
    const firstReq = new NextRequest(
      "http://localhost/api/v1/marketplace/runs",
      {
        method: "POST",
    ...
    expect(paidRes.status).toBe(201);
    const run = await paidRes.json();
    expect(run.payment_ref).toBe("pay_rk_market_pending");
    expect(run.settlement_tx_hash).toBe(
      "0x111122223333444455556666777788889999aaaabbbbccccddddeeeeffff",
    );
  });

  it("rejects paid retry when x402 challenge context no longer matches profile identity snapshot", async () => {
    upsertAgentProfile({
      agent_id: "staking_steward",
      name: "Staking Steward",
      description: "staking",
    ...
    const firstReq = new NextRequest(
      "http://localhost/api/v1/marketplace/runs",
      {
        ...
        body: JSON.stringify({
          hire_id: "hire_context_mismatch_1",
          agent_id: "staking_steward",
          action: "stake",
          params: { pool: "0xpool", amount: "100" },
          billable: true,
    ...
    const payment = createStrictX402Payment(challenge, {
      tongoAddress: "tongo1payer",
      amount: challenge.minAmount,
      replayKey: "rk_context_mismatch_1",
      nonce: "nonce_context_mismatch_1",
    });
    ...
    upsertAgentProfile({
      ...
      endpoint_proofs: [
        {
          endpoint: "https://example.com/staking",
          nonce: "nonce_profile_context_v2",
          digest: "c".repeat(64),
        },
      ],
      pricing: {
  ...
  it("supports status filtering with pagination", async () => {
    ...
    const listReq = new NextRequest(
      "http://localhost/api/v1/marketplace/runs?status=queued&limit=1&offset=0",
      {
        method: "GET",
        headers: {
          "X-API-Key": "test-key-1234567890",
    ...
    expect(listJson.pagination.total).toBe(2);
  });
