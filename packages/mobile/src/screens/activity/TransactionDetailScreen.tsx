...
export default function TransactionDetailScreen({ navigation, route }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      if (isMounted && cachedRow) {
        setMeta(prev => ({
          ...(prev || initialMeta),
          ...(cachedRow as TxMetadataExtended),
        }));
      }
  ...
  const wardNorm =
    meta?.wardAddress?.toLowerCase().replace(/^0x0+/, '0x') || '';
  const walletNorm =
    meta?.walletAddress?.toLowerCase().replace(/^0x0+/, '0x') || '';
  // I'm viewing as the ward if my address matches the ward address (and I'm not the guardian who submitted it)
  const viewingAsWard =
    !!myNorm && !!wardNorm && myNorm === wardNorm && myNorm !== walletNorm;

  const isGuardianWardOp =
    !viewingAsWard &&
    meta?.accountType === 'guardian' &&
    GUARDIAN_WARD_TYPES.includes(meta?.type as any);
  const isWardAdmin = WARD_ADMIN_TYPES.includes(meta?.type as any);
  const isShieldedOp =
    SHIELDED_TYPES.includes(meta?.type as any) && !isGuardianWardOp;
  const isPublicOp =
    (meta?.type === 'erc20_transfer' && !isGuardianWardOp) ||
    isWardAdmin ||
    (viewingAsWard && meta?.accountType === 'guardian');
  ...
  const unitsAmount = toTongoUnitsFromAny(
    amountRaw,
    meta?.amount_unit,
    token,
    meta?.type,
  ...
  if (isShieldDeposit) {
    ...
    signedAmount = isGuardianWardOp
      ? unitLabel(unitsAmount)
      : `${prefix}${unitLabel(unitsAmount)}`;
    if (unitsAmount !== '0') {
  ...
  const hasVoyagerTx = /^0x[0-9a-f]+$/i.test(txHash);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.hero}>
          <View
            style={[
              ...
              (isGuardianWardOp || isWardAdmin) && {
                backgroundColor: wardYellowBg,
                borderColor: wardYellowBorder,
              },
            ]}
          >
            {headerIcon}
          ...
          >
            <View
              style={[
                styles.confirmDot,
                { backgroundColor: statusPill.dotColor },
              ]}
            />
        ...
        <View style={styles.detailsCard}>
          ...
          <View style={styles.detailRow}>
            ...
            <View style={styles.typeRow}>
              <Text style={[styles.detailValue, styles.detailValueType]}>
                {typeValue}
              </Text>
              {meta?.accountType &&
              meta.accountType !== 'normal' &&
              !viewingAsWard ? (
                <View
                  style={[
                    styles.accountTypeBadge,
                    (isGuardianWardOp || meta.accountType === 'ward') && {
                      backgroundColor: wardYellowBg,
                    },
                  ]}
                >
            ...
            <>
              ...
              <View style={styles.detailRow}>
                ...
                <View style={{ alignItems: 'flex-end', gap: 2 }}>
                  ...
                  >
                    {meta?.wardAddress
                      ? truncateMiddle(meta.wardAddress, 8, 6)
                      : 'Unknown'}
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.detailRow}>
                ...
                <Text style={[styles.detailValue, { color: wardYellow }]}>
                  Approved as Guardian
                </Text>
              </View>
            </>
          ) : null}
        ...
        <Text style={styles.hashLabel}>
          {hasVoyagerTx ? 'Transaction Hash' : 'Activity Reference'}
        </Text>
